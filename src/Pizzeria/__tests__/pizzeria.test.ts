...
function setUpPizzeria(): Pizzeria {
  const employees: IEmployees = new Employees();
  const ingredients: IIngredients = new Ingredients();
  const orders: IOrders = new Orders();
  const pizzas: IPizzas = new Pizzas();
  const tables: ITables = new Tables();
  const vouchers: IVouchers = new Vouchers();
  return new Pizzeria(ingredients, employees, tables, vouchers, orders, pizzas);
}

function getPizzeriaObjectWithSettings(withChef: boolean = true): Pizzeria {
  const pizzeria: Pizzeria = setUpPizzeria();
...
describe("Pizzeria module", () => {
  ...
  describe("make new order", () => {
    ...
    it("Success - succesfully created order in queue", () => {
      //given
      const pizzeria: Pizzeria = getPizzeriaObjectWithSettings(false);
